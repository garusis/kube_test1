...
route.post('/', function(req, res){
    ...
    return workshopModel.create(req.body).then(newWShop => res.json(newWShop)).catch(err => res.status(500).send(err.message));
})
...
instanceRouter.delete('/', function(req, res){
    return workshopModel.remove(req.workshop.id).then( value => res.json(value)).catch(err=> res.status(500).send(err.message));
})
